// <auto-generated />
using System;
using Hospital_del_Valle.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace Hospital_del_Valle.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20241204025028_ChangesModelsForInternation")]
    partial class ChangesModelsForInternation
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.11")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("Hospital_del_Valle.Models.Cita", b =>
                {
                    b.Property<int>("CitaID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("CitaID"));

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("FechaCita")
                        .HasColumnType("datetime2");

                    b.Property<int>("MedicoID")
                        .HasColumnType("int");

                    b.Property<string>("Observaciones")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("PacienteID")
                        .HasColumnType("int");

                    b.HasKey("CitaID");

                    b.HasIndex("MedicoID");

                    b.HasIndex("PacienteID");

                    b.ToTable("Citas");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.Habitacion", b =>
                {
                    b.Property<int>("HabitacionID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("HabitacionID"));

                    b.Property<decimal>("CostoDiario")
                        .HasColumnType("decimal(18,2)");

                    b.Property<bool>("Disponible")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<string>("Numero")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("Tipo")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.HasKey("HabitacionID");

                    b.ToTable("Habitaciones");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.HistorialClinico", b =>
                {
                    b.Property<int>("HistorialID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("HistorialID"));

                    b.Property<string>("Diagnostico")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("FechaActualizacion")
                        .HasColumnType("datetime2");

                    b.Property<int?>("HospitalizacionID")
                        .HasColumnType("int");

                    b.Property<string>("NotasClinicas")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("PacienteID")
                        .HasColumnType("int");

                    b.Property<string>("Tratamiento")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("HistorialID");

                    b.HasIndex("HospitalizacionID");

                    b.HasIndex("PacienteID");

                    b.ToTable("HistorialClinico");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.InventarioFarmacia", b =>
                {
                    b.Property<int>("InventarioID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("InventarioID"));

                    b.Property<DateTime>("FechaUltimaActualizacion")
                        .HasColumnType("datetime2");

                    b.Property<int>("MedicamentoID")
                        .HasColumnType("int");

                    b.Property<int>("StockActual")
                        .HasColumnType("int");

                    b.HasKey("InventarioID");

                    b.HasIndex("MedicamentoID");

                    b.ToTable("InventarioFarmacia");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.Medicamento", b =>
                {
                    b.Property<int>("MedicamentoID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("MedicamentoID"));

                    b.Property<string>("Descripcion")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("FechaVencimiento")
                        .HasColumnType("datetime2");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<int>("Stock")
                        .HasColumnType("int");

                    b.HasKey("MedicamentoID");

                    b.ToTable("Medicamentos");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.Notificacion", b =>
                {
                    b.Property<int>("NotificacionID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("NotificacionID"));

                    b.Property<DateTime>("FechaNotificacion")
                        .HasColumnType("datetime2");

                    b.Property<bool>("Leido")
                        .HasColumnType("bit");

                    b.Property<string>("Mensaje")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("UsuarioID")
                        .HasColumnType("int");

                    b.HasKey("NotificacionID");

                    b.HasIndex("UsuarioID");

                    b.ToTable("Notificaciones");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.PacienteHospitalizado", b =>
                {
                    b.Property<int>("HospitalizacionID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("HospitalizacionID"));

                    b.Property<int?>("EnfermeroID")
                        .HasColumnType("int");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime?>("FechaAlta")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("FechaIngreso")
                        .HasColumnType("datetime2");

                    b.Property<int>("HabitacionID")
                        .HasColumnType("int");

                    b.Property<int>("MedicoID")
                        .HasColumnType("int");

                    b.Property<string>("NotasSeguimiento")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("PacienteID")
                        .HasColumnType("int");

                    b.HasKey("HospitalizacionID");

                    b.HasIndex("EnfermeroID");

                    b.HasIndex("HabitacionID");

                    b.HasIndex("MedicoID");

                    b.HasIndex("PacienteID");

                    b.ToTable("PacientesHospitalizados");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.Pago", b =>
                {
                    b.Property<int>("PagoID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("PagoID"));

                    b.Property<string>("Descripcion")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("FechaPago")
                        .HasColumnType("datetime2");

                    b.Property<decimal>("Monto")
                        .HasColumnType("decimal(18,2)");

                    b.Property<int>("PacienteID")
                        .HasColumnType("int");

                    b.HasKey("PagoID");

                    b.HasIndex("PacienteID");

                    b.ToTable("Pagos");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.Prescripcion", b =>
                {
                    b.Property<int>("PrescripcionID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("PrescripcionID"));

                    b.Property<string>("Dosis")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Duracion")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("FechaPrescripcion")
                        .HasColumnType("datetime2");

                    b.Property<string>("Indicaciones")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("MedicamentoID")
                        .HasColumnType("int");

                    b.Property<int>("MedicoID")
                        .HasColumnType("int");

                    b.Property<int>("PacienteID")
                        .HasColumnType("int");

                    b.HasKey("PrescripcionID");

                    b.HasIndex("MedicamentoID");

                    b.HasIndex("MedicoID");

                    b.HasIndex("PacienteID");

                    b.ToTable("Prescripciones");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.PruebaLaboratorio", b =>
                {
                    b.Property<int>("PruebaID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("PruebaID"));

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("FechaSolicitud")
                        .HasColumnType("datetime2");

                    b.Property<int?>("HospitalizacionID")
                        .HasColumnType("int");

                    b.Property<int>("MedicoID")
                        .HasColumnType("int");

                    b.Property<int>("PacienteID")
                        .HasColumnType("int");

                    b.Property<string>("Resultados")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TipoPrueba")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("PruebaID");

                    b.HasIndex("HospitalizacionID");

                    b.HasIndex("MedicoID");

                    b.HasIndex("PacienteID");

                    b.ToTable("PruebasLaboratorio");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.SolicitudMaterial", b =>
                {
                    b.Property<int>("SolicitudID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("SolicitudID"));

                    b.Property<string>("Detalles")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("EnfermeroID")
                        .HasColumnType("int");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("FechaSolicitud")
                        .HasColumnType("datetime2");

                    b.HasKey("SolicitudID");

                    b.HasIndex("EnfermeroID");

                    b.ToTable("SolicitudesMateriales");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.Usuario", b =>
                {
                    b.Property<int>("UsuarioID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("UsuarioID"));

                    b.Property<string>("Apellido")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Contraseña")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Correo")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime>("FechaRegistro")
                        .HasColumnType("datetime2");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Telefono")
                        .IsRequired()
                        .HasMaxLength(15)
                        .HasColumnType("nvarchar(15)");

                    b.Property<string>("TipoUsuario")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("UsuarioID");

                    b.ToTable("Usuarios");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.Cita", b =>
                {
                    b.HasOne("Hospital_del_Valle.Models.Usuario", "Medico")
                        .WithMany("CitasComoMedico")
                        .HasForeignKey("MedicoID")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Hospital_del_Valle.Models.Usuario", "Paciente")
                        .WithMany("CitasComoPaciente")
                        .HasForeignKey("PacienteID")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Medico");

                    b.Navigation("Paciente");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.HistorialClinico", b =>
                {
                    b.HasOne("Hospital_del_Valle.Models.PacienteHospitalizado", "Hospitalizacion")
                        .WithMany("HistorialesClinicos")
                        .HasForeignKey("HospitalizacionID");

                    b.HasOne("Hospital_del_Valle.Models.Usuario", "Paciente")
                        .WithMany()
                        .HasForeignKey("PacienteID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Hospitalizacion");

                    b.Navigation("Paciente");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.InventarioFarmacia", b =>
                {
                    b.HasOne("Hospital_del_Valle.Models.Medicamento", "medicamento")
                        .WithMany()
                        .HasForeignKey("MedicamentoID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("medicamento");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.Notificacion", b =>
                {
                    b.HasOne("Hospital_del_Valle.Models.Usuario", "usuario")
                        .WithMany()
                        .HasForeignKey("UsuarioID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("usuario");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.PacienteHospitalizado", b =>
                {
                    b.HasOne("Hospital_del_Valle.Models.Usuario", "EnfermeroEncargado")
                        .WithMany("HospitalizacionesComoEnfermero")
                        .HasForeignKey("EnfermeroID")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("Hospital_del_Valle.Models.Habitacion", "Habitacion")
                        .WithMany("Hospitalizaciones")
                        .HasForeignKey("HabitacionID")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Hospital_del_Valle.Models.Usuario", "MedicoResponsable")
                        .WithMany("HospitalizacionesComoMedico")
                        .HasForeignKey("MedicoID")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Hospital_del_Valle.Models.Usuario", "Paciente")
                        .WithMany("HospitalizacionesComoPaciente")
                        .HasForeignKey("PacienteID")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("EnfermeroEncargado");

                    b.Navigation("Habitacion");

                    b.Navigation("MedicoResponsable");

                    b.Navigation("Paciente");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.Pago", b =>
                {
                    b.HasOne("Hospital_del_Valle.Models.Usuario", "Paciente")
                        .WithMany()
                        .HasForeignKey("PacienteID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Paciente");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.Prescripcion", b =>
                {
                    b.HasOne("Hospital_del_Valle.Models.Medicamento", "Medicamento")
                        .WithMany()
                        .HasForeignKey("MedicamentoID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Hospital_del_Valle.Models.Usuario", "Medico")
                        .WithMany("PrescripcionesComoMedico")
                        .HasForeignKey("MedicoID")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Hospital_del_Valle.Models.Usuario", "Paciente")
                        .WithMany("PrescripcionesComoPaciente")
                        .HasForeignKey("PacienteID")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Medicamento");

                    b.Navigation("Medico");

                    b.Navigation("Paciente");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.PruebaLaboratorio", b =>
                {
                    b.HasOne("Hospital_del_Valle.Models.PacienteHospitalizado", "Hospitalizacion")
                        .WithMany("PruebasLaboratorio")
                        .HasForeignKey("HospitalizacionID");

                    b.HasOne("Hospital_del_Valle.Models.Usuario", "Medico")
                        .WithMany()
                        .HasForeignKey("MedicoID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Hospital_del_Valle.Models.Usuario", "Paciente")
                        .WithMany()
                        .HasForeignKey("PacienteID")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Hospitalizacion");

                    b.Navigation("Medico");

                    b.Navigation("Paciente");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.SolicitudMaterial", b =>
                {
                    b.HasOne("Hospital_del_Valle.Models.Usuario", "Enfermero")
                        .WithMany()
                        .HasForeignKey("EnfermeroID")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Enfermero");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.Habitacion", b =>
                {
                    b.Navigation("Hospitalizaciones");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.PacienteHospitalizado", b =>
                {
                    b.Navigation("HistorialesClinicos");

                    b.Navigation("PruebasLaboratorio");
                });

            modelBuilder.Entity("Hospital_del_Valle.Models.Usuario", b =>
                {
                    b.Navigation("CitasComoMedico");

                    b.Navigation("CitasComoPaciente");

                    b.Navigation("HospitalizacionesComoEnfermero");

                    b.Navigation("HospitalizacionesComoMedico");

                    b.Navigation("HospitalizacionesComoPaciente");

                    b.Navigation("PrescripcionesComoMedico");

                    b.Navigation("PrescripcionesComoPaciente");
                });
#pragma warning restore 612, 618
        }
    }
}
